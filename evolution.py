...
class Evolution:
    ...
    def rank_based_selection(self, fitness_scores):
        ...
        selected_indices = np.random.choice(len(sorted_population), size=len(self.population), p=probabilities, replace=True)
        ...
        for agent in self.population:
            for gene in agent.genome:
                if np.random.rand() < mutation_rate:
                    agent.gene = np.random.normal(loc=mean, scale=scale)
